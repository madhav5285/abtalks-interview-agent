...
export async function POST(req: Request) {
  try {
    ...
    // Calculate current question/turn count
    const turnCount = Math.floor(history.length / 2) + 1;

    // Track unique curriculum days covered so far
    const coveredDays = Array.from(new Set(history.map((h: InterviewTurn) => h.dayCovered).filter(Boolean)));

    // Rule Check: On or after Question 8, wrap up with Structured Feedback JSON
    ...
    // Select targeted curriculum day to ensure >= 4 days covered
    const targetDayObj = curriculum.topics[(turnCount - 1) % curriculum.topics.length];

    // Dynamic bank of adaptive questions per turn
    const adaptiveQuestions: Record<number, string> = {
      1: "Welcome Madhav! Let's kick off with Day 5. When enforcing structured outputs in production, how do you handle cases where an LLM returns unexpected keys outside your defined JSON schema?",
      ...
      5: "Let's dive into Day 19 (Agentic AI): How do you prevent infinite tool-execution loops in a stateful ReAct agent?",
